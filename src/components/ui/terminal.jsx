...
// Teclas de sonido opcionales (no las usamos — enableSound=false por defecto)
const KEY_SOUNDS_DOWN = {
  A:[31542,85],B:[40621,107],C:[39632,95],D:[32492,85],E:[23317,83],
  F:[32973,87],G:[33453,94],H:[33986,93],I:[25795,91],J:[34425,88],
  K:[34932,90],L:[35410,95],M:[41610,93],N:[41103,90],O:[26309,84],
  P:[26804,83],Q:[22245,95],R:[23817,92],S:[32031,88],T:[24297,92],
  U:[25313,95],V:[40136,94],W:[22790,89],X:[39148,76],Y:[24811,93],
  Z:[38694,80]," ":[51541,144],"-":[42594,90],"@":[23317,83],"/":[42594,90],
  ".":[42594,90],":":[42594,90],"0":[26309,84],"1":[25313,95],"2":[23317,83],
  "3":[23817,92],"4":[24297,92],"5":[24811,93],"6":[25313,95],"7":[25795,91],
  "8":[26309,84],"9":[26804,83],Enter:[19065,110],
};
const KEY_SOUNDS_UP = {
  A:[31632,80],B:[40736,95],C:[39732,85],D:[32577,80],E:[23402,80],
  F:[33063,80],G:[33553,85],H:[34081,85],I:[25890,85],J:[34515,85],
  K:[35027,85],L:[35510,85],M:[41710,85],N:[41198,85],O:[26394,80],
  P:[26889,80],Q:[22345,85],R:[23912,85],S:[32121,80],T:[24392,85],
  U:[25413,85],V:[40236,85],W:[22880,85],X:[39228,70],Y:[24911,85],
  Z:[38779,75]," ":[51691,130],"-":[42689,85],"@":[23402,80],"/":[42689,85],
  ".":[42689,85],":":[42689,85],"0":[26394,80],"1":[25413,85],"2":[23402,80],
  "3":[23912,85],"4":[24392,85],"5":[24911,85],"6":[25413,85],"7":[25890,85],
...
function useAudio(enabled) {
  ...
  const down = (key) => playSound(KEY_SOUNDS_DOWN[key.toUpperCase()] || KEY_SOUNDS_DOWN[key]);
  const up   = (key) => playSound(KEY_SOUNDS_UP[key.toUpperCase()]   || KEY_SOUNDS_UP[key]);
  return { down, up };
}
...
// Colores adaptados a la paleta Bluepeak
const tokenColors = {
  command:  "text-[#8494F9]",   // accent-light — comandos son lo más prominente
  flag:     "text-sky-400",
  string:   "text-amber-300",
  number:   "text-[#5B6EF5]",   // accent
  operator: "text-neutral-500",
  path:     "text-cyan-300",
  variable: "text-[#5B6EF5]",   // accent
...
export function Terminal({
  ...
  useEffect(() => {
    ...
    if (charIdx < currentCommand.length) {
      ...
      const t = setTimeout(() => {
        ...
        setCharIdx(c => c + 1);
      }, typingSpeed + Math.random() * 28);
      return () => clearTimeout(t);
    } else {
      down("Enter");
      const t = setTimeout(() => { up("Enter"); setPhase("executing"); }, 80);
      return () => clearTimeout(t);
    }
  ...
  const prompt = (
    <span className="text-neutral-600">
      <span className="text-[#8494F9]">{username}</span>
  ...
  return (
    <div
      ref={containerRef}
      className={cn("mx-auto w-full px-0 font-mono text-xs", className)}
    >
      <div className="overflow-hidden rounded-sm border border-neutral-800 bg-[#080C14] shadow-2xl">
        {/* Barra de título */}
        <div className="flex items-center gap-2 bg-neutral-900 px-4 py-3 border-b border-neutral-800">
          ...
          <div className="flex-1 text-center">
            <span className="text-xs text-neutral-600 tracking-wide">{username} — bash</span>
          </div>
        ...
        <div
          ref={contentRef}
          className="no-visible-scrollbar h-72 overflow-y-auto p-5 font-mono"
        >
          {lines.map((line, i) => (
            <div key={i} className="leading-relaxed whitespace-pre-wrap mb-0.5">
              {line.type === "command" ? (
                <span>
                  {prompt}
                ...
                <span className="text-neutral-500 pl-4">{line.content}</span>
              )}
            </div>
          ))}
          ...
          {phase === "typing" && (
            <div className="leading-relaxed whitespace-pre-wrap">
              ...
              <span className="ml-0.5 inline-block h-3.5 w-1.5 bg-[#8494F9] align-middle opacity-90" />
            </div>
          )}
          ...
          {(phase === "done" || phase === "pausing" || phase === "outputting") && (
            <div className="leading-relaxed whitespace-pre-wrap">
              ...
              <span
                className={cn(
                  "inline-block h-3.5 w-1.5 bg-[#8494F9] align-middle transition-opacity duration-100",
